...
class Circuit:
    ...
    def display(self):
        res = ""
        for villeTxt in range(len(self.circuit)) :
            res += " " + str(self.circuit[villeTxt].nom)
        res += " "
        return res
...
if __name__ == '__main__':
    ...
    # on cree nos villes
    ville1 = Ville(3.002556, 45.846117, 'A')
    gc.ajouterVille(ville1)
    ville2 = Ville(-0.644905, 44.896839, 'B')
    gc.ajouterVille(ville2)
    ville3 = Ville(-1.380989, 43.470961, 'C')
    gc.ajouterVille(ville3)
    ville4 = Ville(1.376579, 43.662010, 'D')
    gc.ajouterVille(ville4)
    ville5 = Ville(5.337151, 43.327276, 'E')
    gc.ajouterVille(ville5)
    ville6 = Ville(7.265252, 43.745404, 'F')
    gc.ajouterVille(ville6)
    ville7 = Ville(-1.650154, 47.385427, 'G')
    gc.ajouterVille(ville7)
    ville8 = Ville(-1.430427, 48.197310, 'H')
    gc.ajouterVille(ville8)
    ville9 = Ville(2.414787, 48.953260, 'I')
    gc.ajouterVille(ville9)
    ville10 = Ville(3.090447, 50.612962, 'J')
    gc.ajouterVille(ville10)
    ville11 = Ville(5.013054, 47.370547, 'K')
    gc.ajouterVille(ville11)
    ville12 = Ville(4.793327, 44.990153, 'L')
    gc.ajouterVille(ville12)
    ville13 = Ville(2.447746, 44.966838, 'M')
    gc.ajouterVille(ville13)
    ville14 = Ville(1.750115, 47.980822, 'N')
    gc.ajouterVille(ville14)
    ville15 = Ville(4.134148, 49.323421, 'O')
    gc.ajouterVille(ville15)
    ville16 = Ville(7.506950, 48.580332, 'P')
    gc.ajouterVille(ville16)
    ville17 = Ville(1.233757, 45.865246, 'Q')
    gc.ajouterVille(ville17)
    ville18 = Ville(4.047255, 48.370925, 'R')
    gc.ajouterVille(ville18)
    ville19 = Ville(0.103163, 49.532415, 'S')
    gc.ajouterVille(ville19)
    ville20 = Ville(-1.495348, 49.667704, 'T')
    gc.ajouterVille(ville20)
    ville21 = Ville(-4.494615, 48.447500, 'U')
    gc.ajouterVille(ville21)
    ville22 = Ville(-0.457140, 46.373545, 'V')
    ...
    for i in range(0, 100):
        pop = ga.evoluerPopulation(pop)
        print(str(i) + " " + str(pop.getFittest().display()) + str(pop.getFittest().getDistance()))
